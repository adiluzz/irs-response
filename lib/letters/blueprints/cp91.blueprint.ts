/**
 * CP91 Blueprint – Final Notice Before Levy
 */
...
export const cp91Blueprint: Blueprint = {
  noticeType: 'CP91',
  family: 'levy_intent',
  ...
  build(ctx: LetterContext) {
    ...
    // Validator requires city/state/zip — so we supply placeholders if missing.
    ...
    if (!result.success) {
      throw new Error(`CP91 composition failed: ${result.errors.join(', ')}`)
    }
    ...
    return {
      reLine: `CP91 – Final Notice Before Levy – Tax Year ${
        ctx.taxYear || '[year]'
      } – ${ctx.idValue || '[ID]'}`,
