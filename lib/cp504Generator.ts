...
import type { CP504Data } from '@/types'

export interface CP504Options {
  appendReferences?: boolean
  letterDate?: string
  irsAddress?: string
}

export interface CP504Result {
  letterBody: string
...
function getTaxpayerName(data: CP504Data): string {
  const t = asAny(data).taxpayer ?? {}
  return (t.fullName ?? t.name ?? '').toString().trim()
}

function getTaxpayerSSN(data: CP504Data): string {
  const t = asAny(data).taxpayer ?? {}
  return (t.ssn ?? '').toString().trim()
}

function getTaxpayerAddressBlock(data: CP504Data): string {
  const t = asAny(data).taxpayer ?? {}
...
function getTaxYear(data: CP504Data): string {
  const v = asAny(data).taxYear
  if (typeof v === 'number' && Number.isFinite(v)) return String(v)
  return (v ?? '').toString().trim()
}

function getLetterDate(data: CP504Data, options?: CP504Options): string {
...
function getIRSAddress(options?: CP504Options): string {
  const fromOptions = (options?.irsAddress ?? '').toString().trim()
  if (fromOptions) return fromOptions
  return `Internal Revenue Service`
}

function buildHeader(data: CP504Data, options?: CP504Options): string {
...
function buildBody(data: CP504Data): string {
  const noticeDate = (asAny(data).noticeDate ?? '').toString().trim()
  ...
  return `I am writing in response to the CP504 notice${noticeDateText}, Notice Number ${noticeNumber}, concerning my ${taxYear} federal income tax liability.
...
function getAttachments(_: CP504Data): string[] {
  return ['Copy of CP504 Notice', 'Supporting documentation (if applicable)']
}

function getReferences(_: CP504Data): string[] {
  return [
...
function buildClosing(data: CP504Data): string {
  const name = getTaxpayerName(data)
  const ssn = getTaxpayerSSN(data)
...
export function generateCP504Letter(data: CP504Data, options: CP504Options = {}): CP504Result {
