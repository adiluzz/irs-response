...
$root = "C:\tac-response"
$payload = Join-Path $root "PHASE3C_PAYLOAD.TXT"

if (!(Test-Path $payload)) {
  throw "PHASE3C_PAYLOAD.TXT not found in C:\tac-response"
}
...
if ($files.Count -eq 0) {
  throw "No files parsed from PHASE3C payload."
}
...
Write-Host "`nBatch 3C applied. Next: npm run build"
