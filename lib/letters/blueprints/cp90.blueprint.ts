/**
 * CP90 Blueprint – Final Notice of Intent to Levy and Notice of Your Right to a Hearing
 */
...
export const cp90Blueprint: Blueprint = {
  noticeType: 'CP90',
  family: 'levy_intent',
  ...
  build(ctx: LetterContext) {
    ...
    if (!result.success) {
      throw new Error(`CP90 composition failed: ${result.errors.join(', ')}`)
    }
    ...
    return {
      reLine: `CP90 – Final Notice of Intent to Levy – Tax Year ${
        ctx.taxYear || '[year]'
      } – ${ctx.idValue || '[ID]'}`,
