...
$root = "C:\tac-response"
$payload = Join-Path $root "PHASE3D_PAYLOAD.TXT"

if (!(Test-Path $payload)) {
  throw "PHASE3D_PAYLOAD.TXT not found in C:\tac-response"
}
...
if ($files.Count -eq 0) {
  throw "No files parsed from PHASE3D payload."
}
...
Write-Host "`nBatch 3D applied. Next: npm run build"
